...
class EnemyCar:
    def __init__(self, image, lane_index, lane_positions, car_width, car_height, screen_height):#setup
        self.image = image
...
for lane_index in range(4):  #creating opposing cars
    enemy = EnemyCar(opposing_car_image, lane_index, lane_positions, car_width, car_height, SCREEN_HEIGHT)
...
def game_over_screen(score):
    ...
    while True:
        screen.fill(grey)
        screen.blit(game_over_text, (SCREEN_WIDTH // 2 - game_over_text.get_width() // 2, SCREEN_HEIGHT // 3.5))
...
#score
score = 0
score_timer = pygame.time.get_ticks()#tracking when to add point
...
def save_high_score(score):
    with open("high_score.txt", "w") as file:#replacing or creating file for high score
...
while running:
    ...
    #counting and adding score
    if pygame.time.get_ticks() - score_timer >= 1000:#1 point every 1000ms
        score += 1
    ...
    player_rect = pygame.Rect(car_x + 36, car_y + 7, car_width - 72.5, car_height - 9)
    pygame.draw.rect(screen, (0, 255, 0), player_rect, 2)  # green outline


    #check for collision with each enemy car
    for enemy in opposing_cars:
        enemy_rect = pygame.Rect(enemy.x + 36, enemy.y + 7, car_width - 72.5, car_height - 9)#plus and minus to shrink hitboxes
        pygame.draw.rect(screen, (255, 0, 0), enemy_rect, 2)  # red outline
